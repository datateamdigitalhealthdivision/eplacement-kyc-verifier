...
def candidate_outcome(
    ...
    ambiguous_claims = [
        ...
        if int(signal_details.get(signal, {}).get("proof_strength") or 0) == 1
        or bool(signal_details.get(signal, {}).get("ambiguous"))
        or bool(signal_details.get(signal, {}).get("low_confidence"))
    ]
    missing_claims = [
        signal
        for signal in claimed_signals
        if int(signal_details.get(signal, {}).get("proof_strength") or 0) == 0 and signal not in ambiguous_claims
    ]
    reasons: list[str] = []
