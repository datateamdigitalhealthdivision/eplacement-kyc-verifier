...
def build_scoring_sheet(decision_df: pd.DataFrame, settings: AppConfig, job_id: str) -> pd.DataFrame:
    ...
    for _, row in decision_df.iterrows():
        ...
        for suffix in SIGNAL_SUFFIXES:
            ...
            scoring_row[f"proof_strength_{suffix}"] = row.get(f"proof_strength_{suffix}", 0)
            ...
            scoring_row[f"document_type_{suffix}"] = row.get(f"document_type_{suffix}", "")
            scoring_row[f"person_named_{suffix}"] = row.get(f"person_named_{suffix}", "")
            scoring_row[f"person_role_{suffix}"] = row.get(f"person_role_{suffix}", "")
            scoring_row[f"relationship_to_applicant_{suffix}"] = row.get(f"relationship_to_applicant_{suffix}", "")
