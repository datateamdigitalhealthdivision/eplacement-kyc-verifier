...
def merge_results_back(source_df: pd.DataFrame, canonical_df: pd.DataFrame, evidence_results: list[EvidenceResult]) -> pd.DataFrame:
    ...
    for _, canonical_row in canonical_df.iterrows():
        ...
        missing_or_ambiguous = any(
            detail["claimed"] and (detail["missing_proof"] or detail["ambiguous"] or detail["low_confidence"])
        ...
        for signal in SIGNAL_KEYS:
            ...
            if detail["claimed"] and (detail["missing_proof"] or detail["ambiguous"] or detail["low_confidence"]):
